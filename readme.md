...
**👨‍💻 GitHub:** [@novacript](https://github.com/novacript)  
**💬 Telegram:** [@novacript](https://t.me/novacript)
...
### Конкретный пример
```bash
dotnet nuget add source https://nuget.pkg.github.com/novacript/index.json \
  --name github-novacript \
  --username novacript \
...
```bash
dotnet new install novacript.cleanarchitecture.template
```
...
- Обновить шаблон:
  ```bash
  dotnet new install novacript.cleanarchitecture.template --force
  ```
- Удалить шаблон:
  ```bash
  dotnet new uninstall novacript.cleanarchitecture.template
  ```
- Удалить источник:
  ```bash
  dotnet nuget remove source github-novacript
  ```
...
git clone https://github.com/novacript/dotnet-cleanarchitecture-template.git
cd dotnet-cleanarchitecture-template
...
```bash
dotnet add package "Microsoft.Extensions.DependencyInjection.Abstractions" --version "10.0.6"
```

### Для использования `IConfiguration`

```bash
dotnet add package "Microsoft.Extensions.Configuration" --version "10.0.6"
```